    ...
    <main className="p-6 space-y-6">
      ...
      <section className="border rounded-xl p-4 space-y-3">
        ...
        <p className="text-sm text-gray-600">
          Upload files into <strong>Intake</strong> for later sorting. Current batch month:{" "}
          <strong>{month}</strong>
        </p>
          ...
          <div className="space-y-2">
            {queue.map((q) => (
              <div
                ...
                title="Open to categorize"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-medium truncate">{q.raw_name ?? "(no name)"}</div>
                    <div className="text-xs text-gray-600 mt-1">
                      Category: {q.category ?? "—"} • Property: {q.property ?? "—"} • Tags:{" "}
                      {q.tags?.length ? q.tags.join(", ") : "—"}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      {new Date(q.created_at).toLocaleString()} • ID: {q.id}
                    </div>
                  </div>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <button
                      ...
                      title="Categorize this intake item"
                    >
                      Categorize
                    </button>
                  </div>
